...
# Elevation bins along product Z for the DONNER surface stream.
HEIGHT_SURFACE_BIN_CAP = 512


def plane_to_metres(stack: np.ndarray, meta: dict[str, Any] | None = None) -> np.ndarray:
    """`(1, H, W)` or `(H, W)` → `(H, W)` float64 metres; NaN = hole."""
    a = np.asarray(stack)
    if a.ndim == 3:
        if a.shape[0] != 1:
            raise ValueError(f"expected (1,H,W) plane, got {a.shape}")
        plane = a[0]
    elif a.ndim == 2:
        plane = a
    else:
    ...
    meta = meta or {}
    mode = meta.get("mode")
    if mode == "f32" or np.issubdtype(plane.dtype, np.floating):
        out = np.asarray(plane, dtype=np.float64)
        return out

    z0 = float(meta.get("z0_m") or 0.0)
    scale = float(meta.get("scale_m") or 1.0)
    codes = np.asarray(plane)
    out = np.full(codes.shape, np.nan, dtype=np.float64)
    valid = codes > 0
    out[valid] = z0 + codes[valid].astype(np.float64) * scale
    return out


def height_surface_bin_count(
    h_min: float,
    h_max: float,
    *,
    bins: int | None = None,
    bin_cap: int = HEIGHT_SURFACE_BIN_CAP,
) -> int:
    """≈1 bin per metre of relief, capped."""
    if bins is not None:
        return max(2, min(int(bin_cap), int(bins)))
    span = max(0.0, float(h_max) - float(h_min))
    raw = max(2, int(round(span)) + 1)
    return min(max(2, int(bin_cap)), raw)


def height_surface_thw(
    metres: np.ndarray,
    *,
    bins: int | None = None,
    bin_cap: int = HEIGHT_SURFACE_BIN_CAP,
) -> tuple[np.ndarray, dict[str, Any]]:
    """Build a sparse elevation **surface** as a dense Viewer Contract stack.

    Shape ``(nZ, H, W)`` uint16: at most **one** non-zero per map column
    (no filled pillars). Code ``0`` = empty; ``v = bin_index + 1`` for Colormap.
    Allocates and returns whatever size the selection needs — no preflight refuse.
    """
    z = np.asarray(metres, dtype=np.float64)
    if z.ndim != 2:
        raise ValueError(f"height surface needs (H,W) metres, got {z.shape}")
    h, w = int(z.shape[0]), int(z.shape[1])
    valid = np.isfinite(z)
    if not np.any(valid):
        empty = np.zeros((1, h, w), dtype=np.uint16)
        return empty, {
            "axes": "THW",
            "kind": "height_surface",
            "bins": 1,
            ...
            "z_max_m": float("nan"),
            "occupied": 0,
            "nbytes": int(empty.nbytes),
        }

    h_min = float(np.nanmin(z))
    h_max = float(np.nanmax(z))
    n_bins = height_surface_bin_count(h_min, h_max, bins=bins, bin_cap=bin_cap)

    if h_max > h_min:
        u = np.zeros(z.shape, dtype=np.float64)
        np.divide(z - h_min, h_max - h_min, out=u, where=valid)
        zi = np.full(z.shape, -1, dtype=np.int32)
        zi[valid] = np.clip(
            np.rint(u[valid] * (n_bins - 1)), 0, n_bins - 1
        ).astype(np.int32)
    else:
        zi = np.full(z.shape, -1, dtype=np.int32)
        zi[valid] = 0

    vol = np.zeros((n_bins, h, w), dtype=np.uint16)
    ys, xs = np.nonzero(valid)
    codes = (zi[ys, xs] + 1).astype(np.uint16)
    vol[zi[ys, xs], ys, xs] = codes
    occupied = int(ys.size)
    meta = {
        "axes": "THW",
        "kind": "height_surface",
        "dtype": "uint16",
        ...
        "reconstruct": "one voxel per column; v = elevation bin + 1; 0 = empty",
    }
    return np.ascontiguousarray(vol), meta


def height_surface_from_plane(
    stack: np.ndarray,
    ...
    *,
    ...
    """`(1, H, W)` mosaic plane → `(nZ, H, W)` surface for the DONNER hub."""
    metres = plane_to_metres(stack, meta)
    surface, s_meta = height_surface_thw(metres, bins=bins, bin_cap=bin_cap)
    out_meta = {**(meta or {}), **s_meta, "plane_shape": list(np.asarray(stack).shape)}
    return surface, out_meta
